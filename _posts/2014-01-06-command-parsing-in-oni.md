...
#### Fun Fact
During testing, the `shady toolbox` actually revealed a subtle bug in the parser - it was ignoring stuff (!) and it was not immediately clear why. Turnes out the parser was a little too eager to parse things as prepositions. It parsed the `to` part of the `toolbox` as a preposition and then decided it had it would not have to parse further (it was trying to parse a direct object). The solution was simple: add a space to the binary match expression for prepositions. This is still not fail safe but you have to really try to get the parser to screw up now.